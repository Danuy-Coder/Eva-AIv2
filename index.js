...
function startProxyServer() {
  ...
  // CORS — bisa dipanggil dari web manapun
  ...
  function authCheck(req, res, next) {
    ...
    if (!token || token !== cfg.proxyToken) {
      return res.status(401).json({ status: false, message: 'Unauthorized. Sertakan x-api-key yang valid.' })
    }
    next()
  }

  // ── ROUTE: GET /ai ──────────────────────────────────
  // Contoh: GET /ai?text=siapa einstein&token=TOKENMU
  ...
  // ── ROUTE: POST /ai ─────────────────────────────────
  // Body: { "text": "pertanyaan", "logic": "opsional" }
  // Header: x-api-key: TOKENMU
  ...
  app.get('/', (req, res) => {
    res.json({
      ...
      endpoints: {
        'GET  /ai?text=...&token=...': 'Chat AI via query',
        'POST /ai (body: {text}) + header x-api-key': 'Chat AI via body'
      }
    })
  })
  ...
  app.listen(cfg.proxyPort, () => {
    ...
    console.log(`   Endpoint: https://cynix.tokopanel.my.id/ai\n`)
  })
}

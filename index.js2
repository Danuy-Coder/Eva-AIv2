...
│
│ 🖼️ *Media*
│ ${p}pap <keyword>   — kirim foto Pinterest
  ...
  app.get('/', (req, res) => {
    res.json({
      ...
      endpoints: {
        ...
        'GET /pinterest?text=...&token=...': 'Cari foto Pinterest'
      }
    })
  })
      ...
      switch (cmd) {
        ...
        case 'anime': {
          ...
          try {
            ...
            // Ambil array URL gambar dari response
            const data = res.data
            ...
            // Kirim sebagai carousel (max 10 foto)
            ...
            for (const url of urls) {
              ...
              try {
                ...
                mediaMessages.push({ image: buf, caption: '' })
              } catch {}
            }

            if (mediaMessages.length === 0) {
              await react('❌')
              await reply('Gagal mengambil gambar.')
              break
            }

            await react('✅')
            // Kirim satu per satu (WA tidak support true carousel di bot)
            await reply(`🖼️ *${keyword}* — ${mediaMessages.length} foto ditemukan`)
            for (const msg of mediaMessages) {
              await sock.sendMessage(from, msg, { quoted: m })
              await new Promise(r => setTimeout(r, 500)) // delay biar ga spam
            }
          } catch (e) {
            await react('❌')
            await reply(cfg.msg.error)
